...
- [ ] **Add two products** with exact ids:
  - `ivy_weekly` ($8.99 / week)
  - `ivy_annually` ($374.99 / year)
- [ ] **Create one entitlement** called `pro`; attach both products.
- [ ] **Create the default offering** with a "Weekly" package linked to `ivy_weekly` and an "Annual" package linked to `ivy_annually`. Order Annual first.
...
- [ ] **Subscription group** called `ivyos`. Add the two auto-renewable subscriptions with ids matching RevenueCat: `ivy_weekly` and `ivy_annually`.
