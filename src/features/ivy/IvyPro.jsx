...
export default function IvyPro() {
  ...
  // This screen fits the viewport exactly: no page scroll, the composer sits
  // right above the tab bar. body.ivy-fill (global.css) turns the shell's
  // wrapper chain into a definite height so the panel below can simply flex
  // into it. Nothing is measured, so there is nothing to get wrong.
  useLayoutEffect(() => {
    document.body.classList.add('ivy-fill');
    return () => document.body.classList.remove('ivy-fill');
  }, []);
  ...
  return (
    <div className="ivy-fill-page" style={{
      display: 'grid',
...
function ModeChip({ mode, modeError, model }) {
  ...
  // Say what is actually wrong. "no API key" used to show for every
  // failure, including an out-of-credit account with a perfectly good key.
  const noKey = modeError === 'no-api-key';
  const capped = /cap/.test(modeError || '');
  const shortReason = live ? shortModelLabel(model) : noKey ? 'no API key' : capped ? 'daily limit reached' : 'Claude error';
  const tooltip = live
    ? 'Connected to Anthropic - replies are generated by Claude.'
    : noKey
      ? 'Set ANTHROPIC_API_KEY in Vercel and redeploy.'
      : capped
        ? "Today's Ivy limit is used up; live answers return tomorrow."
        : `Claude couldn't answer: ${modeError || 'unknown error'}. Details: joinivy.ai/api/admin/prod-readiness`;
  return (
    <div title={tooltip} style={{
      ...
      <span style={{ fontWeight: 600 }}>{live ? 'Claude live' : 'Canned replies'}</span>
