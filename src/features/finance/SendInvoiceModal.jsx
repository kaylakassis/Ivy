// Pick a client to send an invoice to.
...
export default function SendInvoiceModal({ invoice, onSend, onClose }) {
  ...
  const sendToCurrent = async () => {
    ...
    setBusy(true); setErr(null);
    try { await onSend(invoice.clientId); }
    catch (e) { setErr(e.message || 'Could not send'); setBusy(false); }
  };
    ...
    <div onClick={onClose} style={{
      ...
      <div onClick={(e) => e.stopPropagation()} className="card scroll" style={{
        ...
        <div style={{ padding: '18px 20px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: 10 }}>
          <div style={{ flex: 1 }}>
            <div className="metric-label">Send invoice</div>
        ...
        {invoice.clientName && invoice.clientEmail && (
          <div style={{ padding: '12px 16px', borderBottom: '1px solid var(--border)' }}>
            <button onClick={sendToCurrent} disabled={busy}
              className="btn btn-primary" style={{ width: '100%', justifyContent: 'space-between' }}>
              <span>Send to {invoice.clientName} · {invoice.clientEmail}</span>
              <Icons.Arrow size={13}/>
            </button>
            <div style={{ fontSize: 11, color: 'var(--muted)', marginTop: 8, textAlign: 'center' }}>
              or pick a different recipient below
            </div>
          </div>
        )}
        ...
        <div className="scroll" style={{ flex: 1, overflowY: 'auto', minHeight: 200 }}>
          {loading ? (
            ...
            <div style={{ padding: 32, color: 'var(--muted)', fontSize: 13, textAlign: 'center' }}>
              {clients.length === 0
                ? 'Add clients first to send them invoices.'
                : 'Clients need an email on file.'}
            </div>
          ) : candidates.map((c) => {
