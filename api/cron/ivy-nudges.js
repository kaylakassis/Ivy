...
// Per-batch size for the deadline-driven loop. Each "fire" is one push
// notification + one INSERT (ON CONFLICT), so the dominant cost is the
// Postgres round-trip per row. 200 keeps the batch small enough that
// the deadline check inside the loop fires often without thrashing.
...
async function handler(req, res) {
  ...
  try {
    ...
    // Both detectors shard on the same column so a given workspace is
    // always processed by the same shard — keeps the per-workspace
    // dedup (ivy_nudges_fired) free of cross-shard contention.
    const tFilter = shardClause({ shard, shards }, 't.workspace_id');
    const lFilter = shardClause({ shard, shards }, 'l.workspace_id');
    ...
    // versa — each gets up to half of the cron's wall-clock budget.
    const HALF_BUDGET_MS = 135_000;
    ...
    // also require last_message_at > workspace cap. The fire INSERT
    // stamps ivy_nudges_fired so re-running the candidate query in the
    // next batch naturally excludes everything we just fired.
        ...
        const { rows } = await sql.query(
          `SELECT t.workspace_id, t.client_id, t.id AS thread_id,
            ...
            WHERE t.unread_biz > 0
              ...
              AND t.last_message_at <= NOW() - ($1 || ' hours')::interval
    ...
    // archived/lead-only contacts. Same loop pattern: each fire stamps
    // ivy_nudges_fired so the next batch's NOT EXISTS / NULL filter
        ...
        const { rows } = await sql.query(
          `WITH activity AS (
           ...
           SELECT l.workspace_id, l.client_id, l.last_at, c.name AS client_name, c.stage
            ...
            WHERE l.last_at <= NOW() - ($2 || ' days')::interval
              AND l.last_at >  NOW() - ($1 || ' days')::interval
              AND n.workspace_id IS NULL
              AND c.stage = 'active'
              ${lFilter}
