...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Claim the run FIRST (advance the schedule gated on the occurrence
    // count we read) so two concurrent "Run now" clicks can't both issue
    // an invoice. Advance-before-insert mirrors the cron's race-safe
    // ordering: if the insert below fails we've skipped a cycle, never
    // double-issued.
