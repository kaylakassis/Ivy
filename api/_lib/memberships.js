...
//                 insufficient info to upsert (no Ivy metadata, and
...
//     Ivy metadata at all. If the sub's customer maps to a clients
...
export async function applySubscriptionState({ workspaceId, sub, stripeContext }) {
  ...
  if (ours.rows.length === 0) {
    ...
    // Path (b): Stripe-Dashboard-originated subscription with no Ivy
    ...
    // against a customer Ivy has never seen. Fetch the customer
    ...
    if (!client && stripeContext?.secretKey) {
      ...
      // added the contact in Ivy before the Stripe-side subscription)
