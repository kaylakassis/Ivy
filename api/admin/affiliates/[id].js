...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'PATCH') {
      ...
      if ('code' in body) {
        ...
        await recordAudit(req, {
          actor, targetUserId: before.user_id, action: 'affiliate.rotate_code',
          meta: { from: before.code, to: c, affiliateId: id },
        });
      }
      if ('active' in body) {
        await sql`UPDATE affiliates SET active = ${!!body.active}, updated_at = NOW() WHERE id = ${id}`;
        await recordAudit(req, {
          actor, targetUserId: before.user_id, action: 'affiliate.set_active',
          meta: { active: !!body.active, code: before.code, affiliateId: id },
        });
      }
    ...
    if (req.method === 'DELETE') {
      ...
      await recordAudit(req, {
        actor, targetUserId: before.user_id, action: 'affiliate.delete',
        meta: { code: before.code, affiliateId: id },
      });
