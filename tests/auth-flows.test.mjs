...
async function testSignupAndImmediateAccess() {
  ...
  const req = makeReq({ body: { username: 'u' + Math.random().toString(36).slice(2, 10), email, password: 'sufficiently-long-password', name: 'Tester', acceptedTermsVersion: CURRENT_TERMS_VERSION, acceptedPrivacyVersion: CURRENT_PRIVACY_VERSION } });
  const res = makeRes();
...
async function testLoginRejectsWrongPassword() {
  ...
  await signupHandler(makeReq({ body: { username: 'u' + Math.random().toString(36).slice(2, 10), email, password: 'correct-password-1234', name: 'B', acceptedTermsVersion: CURRENT_TERMS_VERSION, acceptedPrivacyVersion: CURRENT_PRIVACY_VERSION } }), signupRes);
...
async function testLoginSuccessIssuesSession() {
  ...
  await signupHandler(makeReq({ body: { username: 'u' + Math.random().toString(36).slice(2, 10), email, password: 'correct-pass-7777', name: 'C', acceptedTermsVersion: CURRENT_TERMS_VERSION, acceptedPrivacyVersion: CURRENT_PRIVACY_VERSION } }), makeRes());
...
async function testLogoutClearsSession() {
  ...
  const signupRes = makeRes();
  await signupHandler(makeReq({ body: { username: 'u' + Math.random().toString(36).slice(2, 10), email, password: 'pass-8888', name: 'D', acceptedTermsVersion: CURRENT_TERMS_VERSION, acceptedPrivacyVersion: CURRENT_PRIVACY_VERSION } }), signupRes);
...
async function testDuplicateEmailSignupRejected() {
  ...
  const first = makeRes();
  await signupHandler(makeReq({ body: { username: 'u' + Math.random().toString(36).slice(2, 10), email, password: 'pass-1234', name: 'E1', acceptedTermsVersion: CURRENT_TERMS_VERSION, acceptedPrivacyVersion: CURRENT_PRIVACY_VERSION } }), first);
  assert(first.statusCode === 201 || first.statusCode === 200, 'first signup OK');

  const dup = makeRes();
  await signupHandler(makeReq({ body: { username: 'u' + Math.random().toString(36).slice(2, 10), email, password: 'pass-5678', name: 'E2', acceptedTermsVersion: CURRENT_TERMS_VERSION, acceptedPrivacyVersion: CURRENT_PRIVACY_VERSION } }), dup);
